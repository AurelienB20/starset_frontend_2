...
const newScreen = () => {
  ...
  const toggleLike = async (item: any) => {
    ...
    const alreadyLiked = item.likedByUser;

    try {
      const endpoint = alreadyLiked ? 'unlike-image' : 'like-image';
      const response = await fetch(`${config.backendUrl}/api/uploads/${endpoint}`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ user_id: userId, image_adress: item.image_url }),
      });
      ...
      if (data.success) {
        setMostLikedImages(prev =>
          prev.map(img =>
            img.image_url === item.image_url
              ? {
                  ...img,
                  likedByUser: !alreadyLiked,
                  like_count: alreadyLiked ? img.like_count - 1 : img.like_count + 1,
                }
              : img
          )
        );
  ...
  const goToChat = (conversation_id: any, contact_profile_picture_url: string) => {
    navigation.navigate({
      ...
      params: {
        conversation_id,
        sender_id: selectedWorker?.account_id,
        sender_type: 'user',
  ...
  const checkConversation = async (worker: any) => {
    try {
      ...
      const person2_id = worker.account_id;
  ...
  const fetchMostLikedImages = async () => {
    try {
      ...
      const data = await response.json();
      if (data.success) {
        // ⚠️ On initialise likedByUser à false (le back ne le renvoie pas encore)
        setMostLikedImages(data.most_liked_images.map((img: any) => ({ ...img, likedByUser: false })));
      }
    } catch (error) {
    ...
    <View style={styles.postContainer}>
      ...
      <View style={styles.postFooter}>
        <View style={styles.footerLeft}>
          ...
          <Text>{item.like_count || 0} likes</Text>
        </View>
